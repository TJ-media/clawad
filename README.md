...
특정 버전을 재현해야 하는 경우에만 `@latest` 대신 버전을 고정합니다(`@clawad/cli@0.1.17`). 사내 방화벽이 npm 레지스트리를 막아 설치되지 않는 환경에서는 [GitHub Release](https://github.com/TJ-media/clawad/releases/latest)의 `clawad-cli.tgz`를 내려받아 같은 명령의 파일 경로로 실행할 수 있습니다. 체크섬 검증과 업데이트·롤백 계약은 [CLI 배포·업데이트 문서](docs/operations/client-distribution.md)에 설명되어 있습니다.
