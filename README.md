...
설치 과정은 작업 활성 구간을 판정할 Claude Code 훅과 사용자 범위 자동 동기화를 등록하고, 데스크탑 오버레이 앱을 함께 설치한 다음 브라우저 로그인을 시작합니다. **Claude Code의 `statusLine` 설정은 건드리지 않습니다.** 제거하면 훅·예약 작업·오버레이 앱을 모두 되돌립니다.
...
특정 버전을 재현해야 하는 경우에만 `@latest` 대신 버전을 고정합니다(`@clawad/cli@0.1.20`). 사내 방화벽이 npm 레지스트리를 막아 설치되지 않는 환경에서는 [GitHub Release](https://github.com/TJ-media/clawad/releases/latest)의 `clawad-cli.tgz`를 내려받아 같은 명령의 파일 경로로 실행할 수 있습니다. 체크섬 검증과 업데이트·롤백 계약은 [CLI 배포·업데이트 문서](docs/operations/client-distribution.md)에 설명되어 있습니다.
...
```text
clawad/
├── client/            # Claude Code 활동 감지 훅, 동기화, 노출 수거, 로그인, 설치·복구
