...
> **현재 상태:** 클로애드는 알파 테스트 단계입니다. 광고 표시는 Windows 데스크탑 오버레이 앱(현재 유일한 표시 창구, CLI 설치 시 함께 설치됨)이 담당하며, Codex·VS Code·Cursor용 광고 어댑터는 아직 제공하지 않습니다. 클로애드는 Claude Code의 `statusLine` 설정을 사용하지 않습니다.
...
특정 버전을 재현해야 하는 경우에만 `latest` 대신 버전 고정 URL(`releases/download/v0.1.12/clawad-cli.tgz`)을 사용합니다. 체크섬 검증과 업데이트·롤백 계약은 [CLI 배포·업데이트 문서](docs/operations/client-distribution.md)에 설명되어 있습니다.
