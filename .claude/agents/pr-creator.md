...
- `gh` CLI가 없으면 커밋·푸시까지만 수행하고, `PR_FAILURE`에 "gh CLI 미설치"와 수동 PR 생성 URL(`https://github.com/TJ-media/{레포}/compare/develop...{브랜치}`)을 담아 반환한다. 우회 설치를 시도하지 않는다.
...
- 베이스 브랜치: `develop` (main은 운영 배포용이라 기능 PR의 대상이 아니다)
