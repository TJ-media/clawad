...
- **프라이버시**: 수집 허용목록 외 데이터는 코드가 접근 자체를 못 하게 설계. 허용목록의 단일 기준은 실제 전송 스키마(`docs/legal/privacy-design.md` §1) — 클라이언트 전송 필드는 serveToken·sequence·machineId·startedAt·endedAt·renderStarted·clientVersion 7개뿐이고 userId는 서버가 인증 세션에서 확정(renderStarted는 CLAW-71 표시 시작 진단 신호, 선택적·판정 미사용). 하드웨어 식별자(MAC·시리얼·UUID)와 **접속 IP**는 제품 이벤트로 수집 금지, 머신 ID는 로컬 랜덤 가명값.
