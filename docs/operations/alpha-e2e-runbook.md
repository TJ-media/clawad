...
1. 기존 설치가 없는 깨끗한 프로필에서 설치하고 활동 감지 훅 등록, 동기화 작업 등록, 오버레이 앱 설치를 확인한다. `settings.json`에 우리 `statusLine`이 **없어야** 한다 (`OS.<os>.INSTALL`).
2. 사용자가 이미 설정한 statusLine을 준비하고 클로애드를 설치·업데이트한다. 그 설정이 **그대로 남아 있는지**와 재업데이트의 멱등성을 확인한다. 0.1.11 이하에서 올라오는 경우에는 우리가 점유했던 슬롯이 설치 전 설정으로 원상복구되는지 확인한다 (`OS.<os>.UPDATE`).
...
5. 제거 후 활동 감지 훅·스케줄·오버레이 앱만 사라지고 사용자 statusLine 설정은 그대로인지 확인한다. 사용자 파일과 append-only 원장은 훼손하지 않는다 (`OS.<os>.UNINSTALL_RESTORE`).
...
이 모드는 기존 PAID/HOUSE 미사용 번들을 폐기한 뒤 TEST 번들만 캐시한다. 오버레이 표시의 `[광고]` 표기, 5초 사실 원장, 서버 승인 이벤트를 확인하고 광고주 과금 원장과 실제 리워드 원장이 생성되지 않았음을 함께 검산한다. 리허설이 끝나면 API 게이트를 `false`로 복구하고 클라이언트 환경 변수를 제거한 뒤 일반 sync를 실행해 TEST 미사용 번들을 폐기한다.
...
3. 세 OS에서 클라이언트를 제거하고 사용자 statusLine 설정이 그대로인지 다시 확인한다.
