...
[sync 데몬]  GET /v1/ad-decision (인증)
     ...
     → 클라이언트: 로컬 캐시에 저장  (오버레이는 캐시만 읽음 = 무네트워크)
[오버레이]   광고 표시 → 표시 사실만 스풀 파일로 기록 (판정하지 않음)
[수거]       overlay-events.js: 표시 구간 ∩ 활성 구간 ≥ 5초 → 사실 이벤트 생성(로컬 원장)
...
- 가장 먼저 시작된 정상 노출 한 건을 인정하고, 시작 시각 동률이면 서버 멱등 키 사전순으로 선택한다. 업로드 순서나 서버 도착 순서는 판정에 사용하지 않는다. 나머지 → `CONCURRENT_USER_IMPRESSION`.
- **동시 노출 거절은 제재가 아니다.** 사용자 제재 점수에 넣지 않는다. 과금·리워드·유효 노출·리포트 유효 노출에서 제외하되, 원본 수신 수와 무효 사유 통계에는 포함한다.
- 동시성 안전: 서로 다른 기기·서버 인스턴스의 거의 동시 도착, 오프라인 배치 업로드 순서 변화, 동일 이벤트 재전송에도 최종 승인 집합이 같다. **PostgreSQL 트랜잭션 + 계정 잠금** 안에서 겹침 연쇄 전체를 `(startedAt, idempotencyKey)` 순으로 재투영한다.
- 늦게 도착한 이벤트가 기존 승자를 바꾸면 원본 이벤트는 수정하지 않는다. 판정 전이 원장을 append하고 기존 과금은 `REFUND`, 새 승자는 `CAPTURE`, 이미 생긴 리워드는 별도 재투영 조정 행으로 상쇄·복원한다. 이 정정은 IVT나 사용자 제재가 아니다.
...
- 현재 Claude Code 훅 신호와 오버레이의 표시 구간만으로는 포커스/가시성(백그라운드 여부)을 신뢰성 있게 판단하기 어렵다.
