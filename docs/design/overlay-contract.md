...
**`adGapMs`는 선택 항목이다 (CLAW-135).** 오버레이가 CLI 업데이트를 함께 위임하더라도 위임 실패는
오버레이 자체 업데이트를 막지 않으므로 두 구성 요소의 버전이 일시적으로 어긋날 수 있기 때문이다.

- 오버레이 업데이트 승인은 CLI 업데이트도 함께 시작하지만 두 갱신은 독립적으로 실패할 수 있다. 따라서
...
- 값이 유효한 `https` URL일 때만 트레이·마스코트 우클릭 메뉴에 **홈페이지 바로가기**를 노출한다.
  메뉴 라벨은 홈페이지지만 키 이름은 `rewardShopUrl` 그대로다 — 구 오버레이가 읽는 필드라 바꾸지 않는다.
- 키가 없거나 `https`가 아니면 홈페이지 메뉴만 숨긴다. 선택 링크의 문제로 광고 정책 캐시 전체를
...
- **sync는 인증 검사보다 먼저 이 포인터와 §2.1 정책 캐시를 쓴다.** 둘 다 인증과 무관한 로컬 파일인데,
  토큰 검사 뒤에 쓰면 한 번도 로그인하지 않은 사용자에게는 영영 쓰이지 않는다 — 오버레이가 §3.4로
  로그인을 시작할 수도, 홈페이지 바로가기를 띄울 수도 없어 로그인 진입점이 터미널 명령만 남는다.
...
### 3.5 오버레이·CLI 통합 갱신 실행 (CLAW-160, CLAW-167)

오버레이는 §3.3의 포인터를 검증한 뒤 같은 디렉터리의 `update.js`에 CLI 갱신을 위임한다.
...
1. 오버레이가 검증한 Node로 `<dirname(script)>/update.js`를 **분리 실행**한다 (완료를 기다리지 않는다)
...
### 3.6 제거 시 연동 정리 진입점 (CLAW-212)

오버레이는 Claude Code 훅·`statusLine`·Codex 훅을 비롯해 여러 에이전트에 자기 항목을 등록한다.
등록한 주체가 오버레이이므로 **되돌리는 주체도 오버레이여야 한다.** clawad는 번들이 노출하는
진입점을 실행할 뿐이고 앱 코드를 import하지 않는다 — Windows에서 `Uninstall Claw-Ad.exe /S`를
실행하는 것과 같은 모양이다 (규칙 §8 경계).

```
<앱 번들>/Contents/Resources/app.asar.unpacked/hooks/cleanup-integrations.js
```

- **macOS 전용 계약이다.** Windows는 NSIS 언인스톨러가 같은 일을 한다.
- 검증된 Node로 인자 배열 실행한다. PATH 검색·shell 문자열 실행은 금지한다.
- 진입점이 없으면(구 오버레이 빌드) 건너뛴다. 제거 자체는 계속한다.

제거 순서는 **종료 → 정리 → 번들 삭제**다. 순서가 바뀌면 각각 이렇게 깨진다:

| 순서 위반 | 결과 |
|---|---|
| 앱이 살아 있는 채로 정리 | 앱이 곧바로 다시 등록한다 (실측) |
| 번들을 먼저 삭제 | 실행할 진입점이 사라진다 |

정리에 실패해도 번들은 지운다 — 남기면 사용자는 앱도 등록도 못 지운다. 대신 무엇이 남았을 수
있는지 알린다. 이 단계가 없던 동안 `statusLine`이 **지워진 앱 경로**를 가리킨 채 남아 Claude
Code가 매 렌더마다 없는 스크립트를 실행했다.
