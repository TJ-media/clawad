...
**알파는 예산을 예약하지 않는다.** 광고 결정 시 가용성만 확인 → 검증 통과 시 확정 차감(capture, 멱등) → 사후 부정 시 광고주 크레딧 복원 + 리워드 회수. 프리페치·미사용 토큰·만료는 예약/해제를 만들지 않는다.

승인 시점에 예산이 소진돼 있으면 광고주 과금 없이 `BUDGET_EXHAUSTED`로 처리하되, **사용자 리워드는 회사 재원으로 적립한다.** 이 손실은 사용자에게 전가하지 않는다.

상세: [ledgers.md](../design/ledgers.md) §예산.
