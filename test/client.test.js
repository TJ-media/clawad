...
test('1P 미만 미전송 적립도 소수점 진행으로 표시한다', () => {
  ...
  for (let i = 0; i < 4; i += 1) {
    const r = run(env, sessionInput());
    assert.strictEqual(r.status, 0);
    steps.push(r.stdout.match(/미전송 예상 ([\d.,]+)P/)[1].replace(',', '.'));
  }
...
test('낮은 유효 정책 단가에서도 양수 적립 진행을 0P로 반올림하지 않는다', () => {
  ...
  const r = run(env, sessionInput());
  assert.strictEqual(r.status, 0);
  assert.match(r.stdout, /미전송 예상 0[.,]001P/);
  assert.doesNotMatch(r.stdout, /미전송 예상 0P/);
});
...
test('오프라인 누적처럼 격차가 크면 예상 적립을 연출 없이 즉시 맞춘다 (CLAW-101)', () => {
  ...
  activateWork(env);
  const r = run(env, sessionInput());
  assert.match(r.stdout, /미전송 예상 30P/, '0.1씩 올리면 300번 걸린다 — 큰 격차는 즉시 맞춘다.');
});
